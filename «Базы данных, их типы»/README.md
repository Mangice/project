# Домашнее задание к занятию «Базы данных, их типы» - Исмаилов Осман
...
- Для работы с финансами необходима СУБД, поддерживающая строгую схему и ACID-совместимость. Именно поэтому реляционные базы данных являются стандартом для бюджетирования и финансового анализа.

1.1.* Хеширование стало занимать длительно время, какое API можно использовать для ускорения работы?

- Увеличить work_mem
  - Увеличить значение параметра work_mem, чтобы сложные операции сортировки и построения хеш-таблиц выполнялись в памяти, а не на диске.
- Использовать hash_mem_multiplier
  - Использовать параметр hash_mem_multiplier, чтобы выделить больше оперативной памяти именно для хеш-операций, не увеличивая общий work_mem для всех процессов.
- Создать расширенную статистику
  - Создать расширенную статистику, чтобы планировщик запросов точнее оценивал объем данных и выбирал более эффективные планы выполнения.
- Применить pgcrypto с AES-NI
  - При криптографическом хешировании применять расширение pgcrypto, которое может задействовать аппаратное ускорение (AES-NI) процессора.
- Включить parallel hash join
  - Активировать параллельные хеш-соединения (parallel hash join), чтобы распределить нагрузку по построению общей хеш-таблицы между несколькими ядрами процессора.
